...
const Notifications = () => {
  ...
  const handleClick = (notif) => {
    setShow(false);
    markAsRead();
    if (notif.type === 'invitation') navigate('/explore');
    else if (notif.type === 'message') navigate(`/messages/${notif.referenceId}`);
    else if (notif.type === 'like' || notif.type === 'comment') navigate(`/?postId=${notif.referenceId}`);
    else navigate('/');
  };
        ...
        <div className="notifications-dropdown">
          {notifications.length === 0 ? (
            ...
            notifications.map(notif => (
              <div key={notif._id} className={`notification-item ${!notif.read ? 'unread' : ''}`} onClick={() => handleClick(notif)}>
